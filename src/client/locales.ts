...
export const zh = {
  ...
  'canvas.sidebarHint': '把生成结果或本地图片放到无限画布上，直接在图上圈选标注，再让 AI 按标注改图（指哪打哪）。',
  'canvas.hintAdd': '工具条「+」从历史 / 画廊选择图片，或直接拖入本地图片',
  'canvas.hintAnnotate': '点选图片后用矩形 / 箭头 / 画笔 / 文字标出要修改的位置',
  'canvas.hintSubmit': '在底部输入修改指令并点「标注改图」，新结果会自动放到原图旁',
  'canvas.add': '添加图片',
...
export const en: Record<keyof typeof zh, string> = {
  ...
  'canvas.sidebarHint': 'Place results or local images on an infinite canvas, mark them up directly, and let the AI edit exactly what you point at.',
  'canvas.hintAdd': 'Use the toolbar “+” to pick history / gallery images, or drop local files',
  'canvas.hintAnnotate': 'Select an image, then mark spots with rect / arrow / brush / text',
  'canvas.hintSubmit': 'Type an instruction below and hit “Annotate & edit” — the result lands beside the source',
...
export const ru: Record<keyof typeof zh, string> = {
  ...
  'canvas.sidebarHint': 'Разместите результаты или локальные картинки на бесконечном холсте, отметьте нужные места — и ИИ изменит именно их.',
  'canvas.hintAdd': '«+» на панели добавит картинки из истории / галереи, либо перетащите файлы',
  'canvas.hintAnnotate': 'Выберите картинку и отметьте места прямоугольником / стрелкой / кистью / текстом',
  'canvas.hintSubmit': 'Введите инструкцию внизу и нажмите «Правка по разметке» — результат появится рядом',
